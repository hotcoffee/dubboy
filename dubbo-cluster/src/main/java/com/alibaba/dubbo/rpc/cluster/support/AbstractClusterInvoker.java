...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
    ...
    /**
     * 使用loadbalance选择invoker.</br> a)先lb选择，如果在selected列表中 或者
     * 不可用且做检验时，进入下一步(重选),否则直接返回</br> b)重选验证规则：selected > available
     * .保证重选出的结果尽量不在select中，并且是可用的
     * 
    ...
    protected void checkInvokers(List<Invoker<T>> invokers, Invocation invocation) {
        if (invokers == null || invokers.size() == 0) {
            throw new RpcException("Failed to invoke the method " + invocation.getMethodName() + " in the service " + getInterface().getName() + ". No provider available for the service " + directory.getUrl().getServiceKey() + " from registry " + directory.getUrl().getAddress() + " on the consumer " + NetUtils.getLocalHost() + " using the dubbo version " + Version.getVersion() + ". Please check if the providers have been started and registered.");
        }
    }
